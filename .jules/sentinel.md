...
## 2026-09-13 - Inline Handler Script Injection via Attribute Unescaping
**Vulnerability:** Interpolating dynamic/escaped strings into inline event handler attributes (`onclick="runAnalysis('` + esc(r.ticker) + `')"`) caused DOM XSS because browser HTML attribute parsers decode HTML entities (`&#39;` -> `'`) before passing attribute values to the JavaScript engine.
**Learning:** `escapeHTML()` protects against standard HTML body injection, but when used inside inline JS event attributes like `onclick="..."`, HTML entity decoding by the browser restores original characters before JS parsing, allowing script execution.
**Prevention:** Avoid inline `onclick` string interpolation. Render identifiers into HTML `data-*` attributes (`data-ticker="..."`) and handle events via DOM event delegation or `addEventListener`.
