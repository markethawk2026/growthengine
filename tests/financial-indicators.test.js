...
test("Market intelligence leaders reuses breadth result without network fetch",async ()=>{
  const fakeBreadth={
    universe:["RELIANCE","TCS","INFY"],
    rows:[
      {ticker:"RELIANCE",name:"Reliance",price:2500,changePct:2.5,volume:10000},
      {ticker:"TCS",name:"TCS",price:3500,changePct:-1.2,volume:20000},
      {ticker:"INFY",name:"Infosys",price:1500,changePct:0.5,volume:15000}
    ],
    advances:2,declines:1,unchanged:0,ratio:2
  };
  const leaders=await sandbox.window.NCMarketIntelligence.leaders(fakeBreadth);
  assert.strictEqual(leaders.gainers[0].ticker,"RELIANCE");
  assert.strictEqual(leaders.losers[0].ticker,"TCS");
  assert.strictEqual(leaders.volumeLeaders[0].ticker,"TCS");
});
