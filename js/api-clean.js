/**
 * NanduChandu Markets - Data Pipelines, Indicators & Math Calculations Layer (CLEANED)
 * Removed: Proxy workarounds, fake data generation, obfuscation
 */
...
// Removed generateDynamicTime() - use real timestamps instead
...
async function yfNews(q) {
  ...
  var fetchPromises = feedSources.map(async function(source) {
    try {
      ...
      console.warn("News feed unavailable: " + source.name);
    }
  });
  ...
  // Return empty array instead of fake data if no articles found
