...
async function leaders(symbols){
  // Bolt Optimization: Re-use pre-fetched breadth object if provided to eliminate redundant network quote calls
  var b=(symbols&&Array.isArray(symbols.rows))?symbols:await breadth(symbols), valid=b.rows.filter(function(r){return r.changePct!==null;});
  return {
    universe:b.universe,
