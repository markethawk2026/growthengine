...
async function render(){
  ...
  try{
    // BOLT OPTIMIZATION: Fetch breadth first, then pass 'b' directly to leaders(b) and fetch sectorPerformance concurrently.
    // This avoids duplicated network requests for the universe symbols in breadth and leaders.
    var b=await NCMarketIntelligence.breadth();
    var results=await Promise.all([NCMarketIntelligence.leaders(b),NCMarketIntelligence.sectorPerformance()]);
    var l=results[0], s=results[1];
    body.innerHTML=
