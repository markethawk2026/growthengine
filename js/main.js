...
async function loadNews(targetTicker) {
  ...
  try {
    ...
    window.ACTIVE_NEWS_POOL.forEach(function(article) {
      layoutHtml += `<div id="card_${article.id}" class="gc news-card" data-news-id="${escapeHTML(article.id)}" style="padding: 12px; cursor: pointer; transition: all 0.2s;"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; gap: 8px;"><span style="color: #0284c7; font-size: 11px; font-weight: 700; text-transform: uppercase;">${escapeHTML(article.source)}</span><span style="color: #64748b; font-size: 10px; font-weight: 500;">${escapeHTML(article.time)}</span></div><p style="font-size: 12.5px; font-weight: 600; line-height: 1.4; margin: 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${escapeHTML(article.headline)}</p></div>`;
    });
    ...
    if (newsSidebar) {
      newsSidebar.addEventListener("click", function(e) {
        ...
        if (card && card.dataset.newsId) {
          window.viewArticleDetail(card.dataset.newsId);
        }
      });
    }
